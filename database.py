...
async def count_products_by_category(category: str):
    async with aiosqlite.connect(DB_NAME) as db:
        cursor = await db.execute("SELECT COUNT(*) FROM products WHERE budget_category = ?", (category,))
        res = await cursor.fetchone()
        return res[0] if res else 0

async def create_order(user_id: int, user_name: str, phone: str, address: str, date: str, product_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        cursor = await db.execute(
...
async def get_product_name(product_id: int):
    async with aiosqlite.connect(DB_NAME) as db:
        cursor = await db.execute("SELECT name, budget_category FROM products WHERE id = ?", (product_id,))
        result = await cursor.fetchone()
        if result:
            name, cat = result
            cat_map = {
                "garden": "Весенний сад", "baskets": "Корзины и Кашпо", 
                "buckets": "Ведра", "small_baskets": "Ведерки и корзинки",
                "tulips": "Тюльпаны", "spring_bouquets": "Весенние букеты"
            }
